...
public class LifeFrame extends JFrame {
	...
	LifeFrame() {
		...
		comboBox.setBounds(285, 200, 30, 20);
...
*/
	...
	private void createButtons() {
		...
		startButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				cells = createTestBoard(panel.getWidth() / panel.getCellSize(),
						panel.getHeight() / panel.getCellSize());
	...
	private void createMenu() {
		...
		//fileMenu.add(new JMenuItem("Choose color of cell!"));
		...
		for (int i = 0; i < cells.length; ++i) {
			for (int j = 0; j < cells[i].length; ++j) {
				...
				if (cells[i][j].isAlive()) {
					if (!mMarkedPoints.contains(p))
						mMarkedPoints.add(p);
				} else {
					if (mMarkedPoints.contains(p))
						mMarkedPoints.remove(p);
				}
			}
		}
